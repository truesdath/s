...
local function SendNotification(text)
    Notification:Notify(
        {Title = "DNS config by cpsa", Description = "cpsa on kord - "..text},
...
if uwuMain.Notifications then
    SendNotification("cpsa on kord - config cpsa")
    wait(3.5)
    SendNotification("cpsa - config by cpsa on kord")
end
